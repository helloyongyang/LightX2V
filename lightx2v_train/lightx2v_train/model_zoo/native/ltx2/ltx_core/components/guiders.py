...
def _params_for_sigma_from_sorted_dict(sigma: float, params_by_sigma: Sequence[tuple[float, MultiModalGuiderParams]]) -> MultiModalGuiderParams:
    """
...
class MultiModalGuider:
    ...
    def calculate(
        ...
        pred = cond + (self.params.cfg_scale - 1) * (cond - uncond_text) + self.params.stg_scale * (cond - uncond_perturbed) + (self.params.modality_scale - 1) * (cond - uncond_modality)
