...
class WeightsProvider:
    ...
    def _copy_to_gpu(
        ...
        *raw*, so a single byte copy of its leading *nbytes* reproduces every view
        ...
        if not cpu_buffer.is_contiguous() or cpu_buffer.dtype != torch.uint8 or cpu_buffer.numel() < nbytes:
            raise ValueError(f"source buffer for block {idx} must be a contiguous uint8 buffer of >= {nbytes} bytes, got {cpu_buffer.dim()}-D {cpu_buffer.dtype} with {cpu_buffer.numel()} elements")
    ...
    def _fuse_block_loras(self, idx: int, weights: dict[str, torch.Tensor]) -> None:
        ...
        for name, tensor in weights.items():
            ...
            products = (ab for ab in (s.get_ab(prefix, device=self._target_device, dtype=agg_dtype) for s in self._lora_sources) if ab is not None)
