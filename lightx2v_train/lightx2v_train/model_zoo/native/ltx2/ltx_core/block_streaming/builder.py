    ...
    def build(
        ...
        **_kwargs: object,
        ...
        if set(block_key_map) != expected_indices:
            ...
            raise ValueError(f"Block weights under prefix '{self.blocks_prefix}.' do not match the {len(blocks)} model blocks: missing indices {missing}, unexpected indices {extra}")
        ...
        if cpu_slots_count >= len(blocks):
            ...
            source, lora_sources = self._build_pinned_source(blocks, dtype, cpu_slots_count, block_key_map, lora_sd_and_strengths)
            non_block_loras = lora_sd_and_strengths
        else:
            reader = DiskTensorReader(checkpoint_paths)
            source, lora_sources = self._build_disk_source(blocks, dtype, cpu_slots_count, reader, block_key_map, prefetch_depth=_PREFETCH_DEPTH)
    ...
    def _build_pinned_source(
        ...
        for block_idx in block_key_map:
            if block_idx >= cpu_slots_count:
                raise ValueError(f"Pinned source requires one CPU slot per block; got block index {block_idx} with only {cpu_slots_count} slots.")
        ...
        should_sync = False
        for key, fused in fuse_lora_weights(block_sd, lora_sd_and_strengths, fuse_rule=self.fuse_rule, preserve_input_device=False):
