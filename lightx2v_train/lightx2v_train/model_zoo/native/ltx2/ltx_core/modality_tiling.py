...
class VideoModalityTilingHelper:
    ...
    def tile_modality(self, modality: Modality, tile: Tile, *, normalize_positions: bool = True) -> tuple[Modality, TilingContext]:
        ...
        if num_total > self._num_generated_tokens:
            ...
            if tile_idx is None:
                raise ValueError(f"Tile with in_coords={tile.in_coords} is not in this helper's tile set; pass a tile obtained from `helper.tiles`.")
        ...
        return tiled, TilingContext(keep_indices=keep_indices, num_total_tokens=num_total, cond_blend_weights=cond_blend_weights)
    ...
    def _generated_token_indices(self, tile: Tile, device: torch.device | None = None) -> torch.Tensor:
        ...
        return (f[:, None, None] * self._latent_shape.height * self._latent_shape.width + h[None, :, None] * self._latent_shape.width + w[None, None, :]).reshape(-1)
