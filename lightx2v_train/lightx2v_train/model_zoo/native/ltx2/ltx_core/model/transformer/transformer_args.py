...
class TransformerArgsPreprocessor:
    ...
    def _prepare_timestep(self, timestep: torch.Tensor, adaln: AdaLayerNormSingle, batch_size: int, hidden_dtype: torch.dtype) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def _prepare_attention_mask(self, attention_mask: torch.Tensor | None, x_dtype: torch.dtype) -> torch.Tensor | None:
        ...
        return (attention_mask - 1).to(x_dtype).reshape((attention_mask.shape[0], 1, -1, attention_mask.shape[-1])) * torch.finfo(x_dtype).max

    def _prepare_self_attention_mask(self, attention_mask: torch.Tensor | BlockMask | None, x_dtype: torch.dtype) -> torch.Tensor | BlockMask | None:
        ...
        """
        if attention_mask is None or isinstance(attention_mask, BlockMask):
            return attention_mask
    ...
    def prepare(
        ...
        batch_size = x.shape[0]
        timestep, embedded_timestep = self._prepare_timestep(modality.timesteps, self.adaln, batch_size, modality.latent.dtype)
        prompt_timestep = None
        if self.prompt_adaln is not None:
            prompt_timestep, _ = self._prepare_timestep(modality.sigma, self.prompt_adaln, batch_size, modality.latent.dtype)
        ...
        cross_attention_mask = self._prepare_self_attention_mask(
            getattr(modality, "cross_attention_mask", None),
