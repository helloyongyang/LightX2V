...
def apply_split_rotary_emb(input_tensor: torch.Tensor, cos_freqs: torch.Tensor, sin_freqs: torch.Tensor) -> torch.Tensor:
    if sin_freqs.shape != cos_freqs.shape:
        raise ValueError(f"apply_split_rotary_emb: sin_freqs.shape {tuple(sin_freqs.shape)} must equal cos_freqs.shape {tuple(cos_freqs.shape)}.")
    ...
    if needs_reshape:
        ...
        if b_freq not in (1, b_in):
            raise ValueError(f"apply_split_rotary_emb: cos_freqs batch ({b_freq}) must be 1 (broadcast) or equal input_tensor batch ({b_in}).")
