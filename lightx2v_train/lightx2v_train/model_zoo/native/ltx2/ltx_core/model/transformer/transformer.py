...
class TransformerOpsConfig:
    ...
    def from_functions(
        ...
        masked_callable = masked_attention.to_callable() if isinstance(masked_attention, MaskedAttentionFunction) else masked_attention
...
class BasicAVTransformerBlock(torch.nn.Module):
    def __init__(
        ...
        self.cross_attention_adaln = (video is not None and video.cross_attention_adaln) or (audio is not None and audio.cross_attention_adaln)
    ...
    def get_ada_values(self, scale_shift_table: torch.Tensor, batch_size: int, timestep: torch.Tensor, indices: slice) -> tuple[torch.Tensor, ...]:
    ...
    def get_av_ca_ada_values(
        ...
        scale_shift_ada_values = self.get_ada_values(scale_shift_table[:num_scale_shift_values, :], batch_size, scale_shift_timestep, scale_shift_indices)
        gate_ada_values = self.get_ada_values(scale_shift_table[num_scale_shift_values:, :], batch_size, gate_timestep, slice(None, None))
    ...
    def forward(  # noqa: PLR0915
        ...
        video_kv_cache: dict | None = None,
        audio_kv_cache: dict | None = None,
        ...
        video_cache_update_info = None
        audio_cache_update_info = None
        ...
        if run_vx:
            ...
            vx_msa_out = self.attn1(
                ...
                current_start=video_current_start,
                defer_cache_update=defer_cache_update,
                detach_cache_update=detach_cache_update,
            )
            if isinstance(vx_msa_out, tuple):
                vx_msa_out, video_cache_update_info = vx_msa_out
        ...
        if run_ax:
            ...
            ax_msa_out = self.audio_attn1(
                ...
                kv_cache=audio_kv_cache,
                current_start=audio_current_start,
                defer_cache_update=defer_cache_update,
                detach_cache_update=detach_cache_update,
            )
            if isinstance(ax_msa_out, tuple):
                ax_msa_out, audio_cache_update_info = ax_msa_out
        ...
        if run_vx:
            vshift_mlp, vscale_mlp, vgate_mlp = self.get_ada_values(self.scale_shift_table, vx.shape[0], video.timesteps, slice(3, 6))
        ...
        if run_ax:
            ashift_mlp, ascale_mlp, agate_mlp = self.get_ada_values(self.audio_scale_shift_table, ax.shape[0], audio.timesteps, slice(3, 6))
        ...
        if video_cache_update_info is not None or audio_cache_update_info is not None:
            return video_out, audio_out, {"video": video_cache_update_info, "audio": audio_cache_update_info}
...
def apply_cross_attention_adaln(
    ...
    shift_kv, scale_kv = (prompt_scale_shift_table[None, None].to(device=x_normed.device, dtype=x_normed.dtype) + prompt_timestep.reshape(batch_size, prompt_timestep.shape[1], 2, -1)).unbind(dim=2)
