...
def _find_temporal_split_size(num_frames: int) -> int:
    ...
    raise ValueError(
        f"Unable to find a valid temporal split size for num_frames={num_frames}. Expected a split size between 3 and 16 such that the final chunk is either exact or has at least 3 frames."
    )
...
def _resnet_block_forward_inplace(
    ...
    if resnet.in_channels != resnet.out_channels:
        raise ValueError(f"In-place resnet forward requires in_channels == out_channels, got {resnet.in_channels} != {resnet.out_channels}")
    ...
    if resnet.timestep_conditioning and timestep is not None:
        ada = resnet.scale_shift_table[None, ..., None, None, None].to(device=interior.device, dtype=interior.dtype) + timestep.reshape(
            interior.shape[0],
            4,
            -1,
            timestep.shape[-3],
...
def _final_norm_and_conv_out(
    ...
    if decoder.timestep_conditioning:
        ...
        ada_values = decoder.last_scale_shift_table[None, ..., None, None, None].to(device=interior.device, dtype=interior.dtype) + embedded_timestep.reshape(
            batch_size,
            2,
            -1,
...
def _memory_efficient_forward(
    ...
    if decoder.timestep_conditioning:
        noise = torch.randn(sample.size(), generator=generator, dtype=sample.dtype, device=sample.device) * decoder.decode_noise_scale
