...
class VideoEncoder(nn.Module):
    ...
    def tiled_encode(
        ...
        if (frames - 1) % VIDEO_SCALE_FACTORS.time != 0:
            ...
            logger.warning(f"Number of frames {frames} of input video is not ({VIDEO_SCALE_FACTORS.time} * k + 1), last {frames_to_crop} frames will be cropped")
...
def to_mapping_operation(
    ...
    def map_op(intervals: DimensionIntervals) -> tuple[list[slice], list[torch.Tensor | None]]:
        ...
        for interval in intervals.intervals:
            output_slice, mask_1d = map_func(interval.start, interval.end, interval.left_ramp, interval.right_ramp, scale)
...
def map_temporal_interval_to_latent(begin: int, end: int, left_ramp: int, right_ramp: int | None = None, scale: int = 1) -> Tuple[slice, torch.Tensor]:
    """
