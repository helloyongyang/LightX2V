...
class ResnetBlock3D(nn.Module):
    ...
    def __init__(
        ...
        self.norm3 = nn.GroupNorm(num_groups=1, num_channels=in_channels, eps=eps, affine=True) if in_channels != out_channels else nn.Identity()
    ...
    def forward(
        ...
        if self.timestep_conditioning:
            ...
            ada_values = self.scale_shift_table[None, ..., None, None, None].to(device=hidden_states.device, dtype=hidden_states.dtype) + timestep.reshape(
                batch_size,
                4,
                -1,
                timestep.shape[-3],
...
class UNetMidBlock3D(nn.Module):
    ...
    def __init__(
        ...
        if timestep_conditioning:
            self.time_embedder = PixArtAlphaCombinedTimestepSizeEmbeddings(embedding_dim=in_channels * 4, size_emb_dim=0)
