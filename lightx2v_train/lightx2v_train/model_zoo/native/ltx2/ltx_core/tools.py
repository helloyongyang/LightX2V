...
class LatentTools(Protocol):
    ...
    def patchify(self, latent_state: LatentState) -> LatentState:
        ...
        if latent_state.latent.shape != self.target_shape.to_torch_shape():
            raise ValueError(f"Latent state has shape {latent_state.latent.shape}, expected shape is {self.target_shape.to_torch_shape()}")
...
class VideoLatentTools(LatentTools):
    ...
    def create_initial_state(
        ...
        if initial_latent is not None:
            assert initial_latent.shape == self.target_shape.to_torch_shape(), f"Latent shape {initial_latent.shape} does not match target shape {self.target_shape.to_torch_shape()}"
        else:
...
class AudioLatentTools(LatentTools):
    ...
    def create_initial_state(
        ...
        return self.patchify(LatentState(latent=initial_latent, denoise_mask=denoise_mask, positions=latent_coords, clean_latent=clean_latent))
