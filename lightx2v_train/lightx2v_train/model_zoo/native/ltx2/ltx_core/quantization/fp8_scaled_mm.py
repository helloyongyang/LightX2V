...
def quantize_weight_to_fp8_per_tensor(weight: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def _quantize(weight_fp32: torch.Tensor, scale: torch.Tensor, fp8_min: torch.Tensor, fp8_max: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
def get_fp8_swap_module_ops(checkpoint_path: str) -> tuple[ModuleOps, ...]:
    ...
    fp8_scale_paths = frozenset(key.removesuffix(".weight_scale") for key in dtypes if key.endswith(".weight_scale") and dtypes.get(key.removesuffix(".weight_scale") + ".weight") == "F8_E4M3")
    if not fp8_scale_paths:
        raise ValueError(
            f"fp8_scaled_mm requires a pre-quantized checkpoint with F8_E4M3 .weight + .weight_scale tensors, but {checkpoint_path!r} has none. Use QuantizationPolicy.fp8_cast() for BF16 checkpoints."
        )
