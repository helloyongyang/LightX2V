...
def _upcast_and_round(weight: torch.Tensor, dtype: torch.dtype, with_stochastic_rounding: bool = False, seed: int = 0) -> torch.Tensor:
    """
...
class Fp8CastLinear(torch.nn.Linear):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:  # noqa: A002, type: ignore[override]
        ...
        b_up = _upcast_and_round(self.bias, input.dtype, self._with_stochastic_rounding, self._seed) if self.bias is not None else None
...
def _amend_forward_with_upcast(model: torch.nn.Module, with_stochastic_rounding: bool = False, seed: int = 0) -> torch.nn.Module:
    """
    ...
    with safetensors.safe_open(str(checkpoint_path), framework="pt", device="cpu") as h:
        ...
        for k in raw_keys:
            ...
            if not k.startswith(_RAW_DIFFUSION_MODEL_PREFIX):
                raise ValueError(f"Scale key {k!r} does not start with the expected raw prefix {_RAW_DIFFUSION_MODEL_PREFIX!r}")
    ...
    * ``.weight_scale`` / ``.bias_scale`` -> drop (the scale is consumed by
    ...
    def _drop_scale(scale_key: str, _value: torch.Tensor) -> list[KeyValueOperationResult]:
        ...
        if param_key not in scales:
            raise ValueError(f"Scale key {scale_key!r} has no matching entry in the prequant scales dict; _read_scales and the loader's rename map have drifted")
        return []
