...
class VideoDmdTrainer(DmdTrainer):
    ...
    def __init__(self, config):
        ...
        parsed = VideoDmdConfig.from_mapping(
            config,
        ...
        self.parsed_video_dmd_config = parsed
    ...
    def _setup_fake_real_resources(self):
        ...
        self.fake_real_model = build_model(model_config)
        self.fake_real_model.load_components(
    ...
    def _extract_real_latents(self, sample):
        with torch.no_grad():
            ...
            return broadcast_sequence_parallel_value(latent)
    ...
    def _real_data_fake_context(self, sample, conditions, region="main"):
        return RealDataFakeStepContext(
            region=region,
            fake_model=self.fake_real_model,
            sample=sample,
            condition=conditions[0],
            negative_condition=conditions[1],
            device=self.model.device,
