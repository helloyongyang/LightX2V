#!/bin/bash
# Run WorldMirror 3D reconstruction (fp8-pertensor quant, single GPU)
#
# 默认开启 Gaussian splats 插值漫游视频渲染（RENDER_VIDEO=1），
# 与 run_worldmirror_recon.sh 行为一致。
set -e
...
if [ -z "${CUDA_VISIBLE_DEVICES+x}" ]; then
    if command -v nvidia-smi >/dev/null 2>&1; then
        ...
        echo "[run_worldmirror_recon_fp8] auto-picked CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
    fi
fi
