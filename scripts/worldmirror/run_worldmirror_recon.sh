#!/bin/bash
# Run WorldMirror 3D reconstruction (fp32 default, single GPU)
# 默认 fp32 精度（1e-3 MAE），~0.77s 推理 / Workspace
#
# 可选：RENDER_VIDEO=1 时，会用预测的 Gaussian splats 额外渲染一段
# 相机插值漫游视频，落盘到 <output>/<case>/<timestamp>/rendered/rendered_rgb.mp4。
# 与 HY-World-2.0 上游 `--save_rendered` 行为一致。
set -e
...
# 未指定 CUDA_VISIBLE_DEVICES 时，自动挑最空闲的单卡，避免与其他
# 进程共卡时出现 CUDA context 冲突 / segfault。
if [ -z "${CUDA_VISIBLE_DEVICES+x}" ]; then
    if command -v nvidia-smi >/dev/null 2>&1; then
        CUDA_VISIBLE_DEVICES=$(nvidia-smi --query-gpu=index,memory.free --format=csv,noheader,nounits \
            | sort -t, -k2 -n -r | head -n1 | awk -F, '{print $1}' | tr -d ' ')
        export CUDA_VISIBLE_DEVICES
        echo "[run_worldmirror_recon] auto-picked CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
    fi
fi
...
# 最新一次运行的时间戳子目录（runner 用 YYYYMMDD_HHMMSS 命名）
