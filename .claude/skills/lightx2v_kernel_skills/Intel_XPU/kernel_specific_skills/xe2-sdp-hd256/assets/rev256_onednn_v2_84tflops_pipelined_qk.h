...
ESIMD_INLINE void flashAttnBMha256Fp16Rev_onednn_v2(
  ...
  for (int32_t outerIter = 0; outerIter < kvOuterLoops; outerIter++) {
    ...
    // Pipelined K loads: K for d+1 loaded after DPAS for d
    // Pre-loaded Q tiles: both Q0 and Q1 loaded before 4 DPAS
    // V prefetch interleaved with QK computation
    // ========================================
    ST_tile = 0;
    ...
    for (int d = 0; d < HD_BLKS; d++) {
      ...
      // Pre-load BOTH Q tiles for 4 consecutive DPAS
      ...
      // 4 consecutive DPAS: 2 K sub-blocks × 2 Q tiles
      {
        auto acc = ST_tile.select<128, 1>(0);
        acc = dpas<8, 8, float, float, fp16, fp16>(
          simd<float, 128>(acc.data()), simd<fp16, 256>(Q_vnni0.data()), K_sb0);
      }
      {
        auto acc = ST_tile.select<128, 1>(128);
        acc = dpas<8, 8, float, float, fp16, fp16>(
          simd<float, 128>(acc.data()), simd<fp16, 256>(Q_vnni0.data()), K_sb1);
      }
      {
        auto acc = ST_tile.select<128, 1>(256);
        acc = dpas<8, 8, float, float, fp16, fp16>(
          simd<float, 128>(acc.data()), simd<fp16, 256>(Q_vnni1.data()), K_sb0);
      }
      {
        auto acc = ST_tile.select<128, 1>(384);
        acc = dpas<8, 8, float, float, fp16, fp16>(
          simd<float, 128>(acc.data()), simd<fp16, 256>(Q_vnni1.data()), K_sb1);
      }

      // Pipeline K load: issue K for d+1 (async, overlaps with next iteration's V prefetch + Q loads)
      if (d < HD_BLKS - 1) {
    ...
    // Deferred sum update: after nbarrier_arrive to overlap with barrier propagation
  ...
  // SIMD16 inversion: 2 SIMD16 math.inv instead of 32 scalar math.inv
