...
#include "rev256_onednn_v2_86tflops_qk_pipeline.h" // v2 under test
...
bool test_correctness(sycl::queue& q, int q_len, int kv_len, int num_heads_q, int num_heads_kv,
    ...
    q.submit([&](sycl::handler& cgh) {
        cgh.parallel_for(nd_range, [=](sycl::nd_item<2> ndi) SYCL_ESIMD_KERNEL {
            flashAttnBMha256Fp16Rev_onednn_v2_qk_pipeline(
...
int main() {
    ...
    std::cout << "================================================================\n";
    std::cout << "Variant: qk_pipeline (QK[k+1] overlapped with barrier A)\n";
    std::cout << "================================================================\n";
    std::cout << "Device: " << q.get_device().get_info<sycl::info::device::name>() << "\n";
    std::cout << "Q Heads: " << NUM_Q_HEADS << ", KV Heads: " << NUM_KV_HEADS << ", Head Dim: 256\n\n";

    // ===== Section 1: Correctness =====
    std::cout << "--- Correctness (qk_pipeline vs CPU fp32 ref) ---\n";
    int pass = 0, total = 0;
    ...
    std::cout << "--- Performance (opt baseline vs qk_pipeline, 100 iters) ---\n";
