...
struct FP16_GEMM_NOPF_V2 {
    ...
    void operator()(sycl::nd_item<1> item) const SYCL_ESIMD_KERNEL {
        ...
        simd<sycl::half, 8*16>  a_tile[2][4];
        simd<sycl::half, 16*16> b_tile[2][4];
        ...
        for (int m = 0; m < 4; m++) {
            ...
            a_tile[0][m] = xesimd::lsc_load_2d<sycl::half, 16, 8, 1, false, false,
        ...
        for (int n = 0; n < 4; n++) {
            ...
            b_tile[0][n] = xesimd::lsc_load_2d<sycl::half, 16, 16, 1, false, true,
        ...
        for (int k = 0; k < K - K_STEP; k += K_STEP) {
            ...
            for (int m = 0; m < 4; m++) {
                payA.set_y((uint32_t)(m_start + m * 8));
                a_tile[1][m] = xesimd::lsc_load_2d<sycl::half, 16, 8, 1, false, false,
                    xesimd::cache_hint::cached, xesimd::cache_hint::cached>(payA);
            }
            ...
            for (int n = 0; n < 4; n++) {
                payB.set_x((uint32_t)(n_start + n * 16));
                b_tile[1][n] = xesimd::lsc_load_2d<sycl::half, 16, 16, 1, false, true,
                    xesimd::cache_hint::cached, xesimd::cache_hint::cached>(payB);
            }
            #pragma unroll
            for (int m = 0; m < 4; m++)
                ...
                for (int n = 0; n < 4; n++)
                    acc[m][n] = xmx::dpas<8, 8, sycl::half, sycl::half,
                        sycl::half, sycl::half>(acc[m][n], b_tile[0][n], a_tile[0][m]);
            ...
            for (int m = 0; m < 4; m++)
                ...
                for (int n = 0; n < 4; n++)
                    acc[m][n] = xmx::dpas<8, 8, sycl::half, sycl::half,
                        sycl::half, sycl::half>(acc[m][n], b_tile[1][n], a_tile[1][m]);
        }
        ...
        for (int i = 0; i < 20; i++)
            q.submit([&](handler& h) {
                h.parallel_for(nd_range<1>(global_size, WG_SIZE),
                    FP16_GEMM_NOPF_V2{d_A, d_B_T, d_C, M, N, K});
            });
        q.wait();
