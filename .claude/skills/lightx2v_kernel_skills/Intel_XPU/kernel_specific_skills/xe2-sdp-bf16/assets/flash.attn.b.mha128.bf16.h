// BF16 Flash Attention kernel - based on flash.attn.b.mha128.gqa.precomputed.h
// All Q/K/V/Output are bf16 in memory. Non-causal only.
// Strategy: use fp16-typed loads (same hardware for 16-bit), bf16 for DPAS only
// QK DPAS: dpas<8,8,float,float,bf16,bf16> (fp32 accumulator for softmax precision)
// S×V DPAS: dpas<8,8,bf16,bf16,bf16,bf16> (bf16 accumulator)
// Softmax: computed in fp32 throughout
// No redundant conversions: loads are fp16-typed (same bits), DPAS reads bf16 bit_cast_view
//
// Optimization: barrier before compensation, interleaved compensation + SxV by l-group,
// early bf16 convert of compensationTemp

using bf16 = sycl::ext::oneapi::bfloat16;

ESIMD_INLINE void flashAttnBMha128Bf16Precomputed(
  uint8_t* qState,
  uint8_t* kState,
  uint8_t* vState,
  uint8_t* normAlpha,
  ...
  for (loopIdx = 0; loopIdx < kvSeqOutLoopCount - 1; loopIdx++) {
    ...
    simd<bf16, 32> compensationTemp;  // declared at loop scope for use after barrier
    tempOutput = 0;
    ...
    // ===== Softmax =====
    {
      ...
      // EARLY bf16 convert — before sum reduction, so sum ALU overlaps with convert latency
  ...
  // ===== LAST LOOP - boundary checking =====
  {
    ...
    auto tempQkAsFp16 = tempOutput.template bit_cast_view<fp16>();
    auto tempQkAsBf16 = tempOutput.template bit_cast_view<bf16>();
    simd<bf16, 32> compensationTemp;  // declared at scope for use after barrier
    tempOutput = 0;
    ...
    // Apply boundary mask, then softmax
    {
      ...
      // EARLY bf16 convert — before sum reduction
      compensationTemp.select<16, 1>(0) = fp32SoftMaxCompensation;
      compensationTemp.select<16, 1>(16) = fp32SoftMaxCompensation;
